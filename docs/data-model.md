...
| `allowed_operations` | AllowedOperation list |
...
AllowedOperation は、記録に対して意図される操作を表す advisory metadata です。詳細は [Operation Permission Model](operation-permission-model.md) を参照してください。

| 値 | 分類 | 意味 |
|---|---|---|
| `view` | read-like | 閲覧・表示する |
| `create` | mutation-like | 新規記録を作成する |
| `edit` | mutation-like | 既存記録を編集する |
| `append` | mutation-like | 過去を黙って書き換えずに追記する |
| `summarize` | read-like | 要約する |
| `reinterpret` | derived-meaning | 後日の解釈・批評・RDE評価に使う |
| `redact` | mutation-like | 匿名化・削除編集する |
| `seal` | mutation-like | 通常アクセスから封印する |
| `export` | disclosure-like | 外部ファイルやpackageへ出力する |
| `inject` | disclosure-like | model contextとして利用する |
| `publish` | disclosure-like | 外部公開する |
...
AllowedOperation は access control enforcement ではありません。後続のdoctor checks、context-use dry-run、security-aware export profilesの基礎です。
...
- Operation-level risk separation is documented.

### Added

- ClassificationLayer 0-4.
- Sensitivity labels.
- Full AllowedOperation vocabulary.
- Read-like / mutation-like / derived-meaning / disclosure-like operation categories.
- LlmPolicy.
- RetentionPolicy.
