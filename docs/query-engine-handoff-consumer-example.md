...
When the checked-in fixture is sufficient, use `docs/query-engine-import-adapter-skeleton.md` as the non-executable starting point for a downstream adapter, or generate a local bundle with `docs/downstream-query-engine-handoff-bundle.md` and evaluate it with `docs/downstream-query-engine-acceptance-checklist.md`.
