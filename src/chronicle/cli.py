...
app = typer.Typer(
    name="chronicle",
    help="Chronicle Core v0.1 — record context, artifacts, decisions, and diffs.",
...
def add_context_cmd(
    ...
    scope: Annotated[ScopeHint, typer.Option("--scope")] = ScopeHint.PROJECT,
    json_output: Annotated[bool, typer.Option("--json")] = False,
) -> None:
...
def record_cmd(
    ...
    try:
        service = ChronicleService()
        event = service.record_event(event_type=type, actor=actor, summary=summary)
        if json_output:
...
def artifact_create_cmd(
    ...
    try:
        ...
        artifact, version = service.create(title=title, artifact_type=type, source_file=file)
        if json_output:
            typer.echo(
                json.dumps(
                    {
...
def decision_record_cmd(
    ...
    artifact: Annotated[Optional[str], typer.Option("--artifact")] = None,
    json_output: Annotated[bool, typer.Option("--json")] = False,
) -> None:
    """Record a Decision."""
    try:
