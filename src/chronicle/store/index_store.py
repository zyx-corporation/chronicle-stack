...
class IndexStore:
    ...
    def save_rde_records(self, records: dict[str, RdeDiffRecord]) -> None:
        self.indexes_dir.mkdir(parents=True, exist_ok=True)
        data = {k: v.model_dump(mode="json") for k, v in records.items()}
        self.rde_index_file.write_text(
            json.dumps(data, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )

    def load_rde_records(self) -> dict[str, RdeDiffRecord]:
        if not self.rde_index_file.exists():
            return {}
