...
class ChronicleUIDataService:
    ...
    def federation_preflight_summary(
        ...
        return {
            ...
            "suggested_package_verify_cli": (
                "chronicle federation package verify --package-dir <package_dir> --json"
            ),
    ...
    def _artifact_linked_contexts(
        ...
        contexts = self.chronicle.index.load_contexts()
        events_by_id = {event.event_id: event for event in self.chronicle.jsonl.read_all()}
        linked_via: dict[str, set[str]] = {}
        source = artifact_record.get("source") or {}
        source_ref = str(source.get("source_ref", "") or "")
        if source_ref.startswith("ctx_") and source_ref in contexts:
            linked_via.setdefault(source_ref, set()).add("artifact_source")
        for version in version_rows:
            event = events_by_id.get(str(version.get("source_event_id", "") or ""))
            if event is None:
                continue
            for context_id in getattr(event, "context_ids", []) or []:
                if isinstance(context_id, str) and context_id in contexts:
                    linked_via.setdefault(context_id, set()).add("version_source_event")

        rows: list[dict[str, Any]] = []
        for context_id in sorted(linked_via):
            ...
            rows.append(
                {
                    ...
                    "linked_via": sorted(linked_via[context_id]),
                }
            )
        return rows

    def _artifact_linked_decisions(self, artifact_id: str) -> list[dict[str, Any]]:
        decisions = sorted(
            (
                decision
                for decision in self.chronicle.index.load_decisions().values()
        ...
        return [
            {
                ...
                "detail_path": f"/api/decisions/{decision.decision_id}",
            }
            for decision in decisions
        ]

    def _artifact_linked_rde_records(self, artifact_id: str) -> list[dict[str, Any]]:
        records = sorted(
            (
                record
                for record in self.chronicle.index.load_rde_records().values()
        ...
        return [
            {
                "rde_record_id": record.rde_record_id,
                "summary": record.summary,
                "created_at": record.created_at.isoformat(),
                "from_version_id": record.from_version_id,
                "to_version_id": record.to_version_id,
                "unresolved_count": len(record.unresolved),
                "deviation_risk_count": len(record.deviation_risks),
                "detail_path": f"/api/rde/{record.rde_record_id}",
            }
            for record in records
        ]

    def _artifact_source_event_summary(self, version_rows: list[dict[str, Any]]) -> dict[str, Any]:
        events_by_id = {event.event_id: event for event in self.chronicle.jsonl.read_all()}
        event_rows: list[dict[str, Any]] = []
        for version in version_rows:
            event_id = str(version.get("source_event_id", "") or "")
        ...
        event_rows.sort(key=lambda item: (item["timestamp"], item["event_id"]))
        latest = event_rows[-1] if event_rows else None
        return {
            "status": "available" if event_rows else "unavailable",
            ...
            "latest_event_summary": latest["summary"] if latest else None,
            "events": event_rows,
            "boundary_note": (
                "Source-event summaries remain derived from artifact version history; they do not replace Chronicle JSONL as authority."
            ),
            "boundary_note_key": "ui.artifact_source_event_summary.note.read_only_derived",
        }

    def _artifact_boundary_summary(
        self,
        artifact_record: dict[str, Any],
        linked_context_rows: list[dict[str, Any]],
    ) -> dict[str, Any]:
        classification = artifact_record.get("classification") or {}
        llm_policy = classification.get("llm_policy") or {}
        linked_visibility_counts = _count_values(linked_context_rows, "visibility_hint")
        source = artifact_record.get("source") or {}
        return {
            "status": "advisory",
            "visibility_hint": artifact_record.get("visibility_hint"),
            "source_type": source.get("source_type") or "unknown",
            "source_ref": source.get("source_ref") or "",
            "allowed_operations": classification.get("allowed_operations", []),
            "llm_policy": llm_policy,
            "linked_context_count": len(linked_context_rows),
            "linked_context_visibility_counts": linked_visibility_counts,
            "boundary_note": (
                "Artifact workbench boundary summary is descriptive only; review, export, and mutation decisions remain explicit separate steps."
            ),
            "boundary_note_key": "ui.artifact_boundary_summary.note.read_only_derived",
        }

    def _artifact_audit_summary(
        self,
        related_record_ids: set[str],
        source_event_ids: set[str],
    ) -> dict[str, Any]:
        matched_rows: list[dict[str, Any]] = []
        for audit_event in self.audit.list_events():
            audit_row = _dump_model(audit_event)
            referenced_records = {
                record_id
                for record_id in audit_row.get("referenced_records", [])
            ...
            matched_record_ids = sorted(related_record_ids.intersection(referenced_records))
            source_event_match = str(audit_row.get("source_event_id", "") or "") in source_event_ids
            if not matched_record_ids and not source_event_match:
                continue
            matched_rows.append(
                {
                    "audit_id": audit_row.get("audit_id"),
                    "operation": audit_row.get("operation"),
                    "result": audit_row.get("result"),
                    "summary": audit_row.get("summary"),
                    "created_at": audit_row.get("created_at"),
                    "detail_path": (
                        f"/api/audit/{audit_row['audit_id']}" if audit_row.get("audit_id") else None
                    ),
                    "matched_record_ids": matched_record_ids,
                    "source_event_matched": source_event_match,
                }
            )
        latest = matched_rows[-1] if matched_rows else None
        return {
            "status": "related_audits_present" if matched_rows else "no_related_audits",
            "audit_event_count": len(matched_rows),
            "operation_counts": _count_values(matched_rows, "operation"),
            "result_counts": _count_values(matched_rows, "result"),
            "latest_audit_id": latest["audit_id"] if latest else None,
            "latest_summary": latest["summary"] if latest else None,
            "latest_detail_path": latest["detail_path"] if latest else None,
            "audits": matched_rows,
            "boundary_note": (
                "Artifact audit summary is a derived reading aid over append-only audit records; it does not authorize downstream operations."
            ),
            "boundary_note_key": "ui.artifact_audit_summary.note.read_only_derived",
        }
    ...
    def detail_payload(self, path: str) -> dict[str, Any] | None:
        ...
        if resource == "events":
            ...
            if record is not None:
                ...
                linked_context_rows = self._artifact_linked_contexts(record, record["versions"])
                linked_decision_rows = self._artifact_linked_decisions(record_id)
                linked_rde_rows = self._artifact_linked_rde_records(record_id)
                source_event_ids = {
                    str(version.get("source_event_id", "") or "")
                    for version in record["versions"]
                    if str(version.get("source_event_id", "") or "")
                }
                related_record_ids = {record_id}
                related_record_ids.update(row["context_id"] for row in linked_context_rows)
                related_record_ids.update(row["decision_id"] for row in linked_decision_rows)
                related_record_ids.update(row["rde_record_id"] for row in linked_rde_rows)
                record["linked_contexts"] = linked_context_rows
                record["linked_decisions"] = linked_decision_rows
                record["linked_rde_records"] = linked_rde_rows
                record["source_event_summary"] = self._artifact_source_event_summary(record["versions"])
                record["boundary_summary"] = self._artifact_boundary_summary(record, linked_context_rows)
                record["audit_summary"] = self._artifact_audit_summary(
                    related_record_ids,
                    source_event_ids,
                )
...
function renderAuditTable(endpoint, rows) {{
  ...
  return renderMultiPanelRoute([
    renderAuditGovernanceSummary(governance),
...
function renderBoundaryTable(endpoint, rows) {{
  const sorted = rows.slice().sort((a, b) => String(b.created_at || '').localeCompare(String(a.created_at || '')));
  return renderMultiPanelRoute([
    renderBoundaryGovernanceSummary(sorted),
    tableHtml([
      label('label.table_detail', 'Detail'),
      label('label.table_status', 'Status'),
      label('label.table_operation', 'Operation'),
...
function renderLifecycleTable(endpoint, rows) {{
  const sorted = rows.slice().sort((a, b) => String(b.created_at || '').localeCompare(String(a.created_at || '')));
  return renderMultiPanelRoute([
    renderLifecycleGovernanceSummary(sorted),
    tableHtml([
      label('label.table_detail', 'Detail'),
      label('label.table_event', 'Event'),
      label('label.table_status', 'Status'),
...
function workspaceLatestResponseLine(path, labelKey, fallbackLabel) {{
  return navigationCluster([
    latestResponseButton(path, labelKey, fallbackLabel),
  ]);
}}
function workspaceSummaryLines(summaryPairs) {{
  return (summaryPairs || []).map(item => summaryJsonLine(item.label, item.value)).join('');
}}
